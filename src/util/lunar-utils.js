...
export function leapDays(year) {
  ...
  // 获取闰月的天数
  const leapLunar = Lunar.fromYmd(year, leapMonth, 1, 1);
  return leapLunar.getMonthDays();
}
...
export function formatLunarDate(lunar) {
  return `农历${lunar.ganZhi}年（${lunar.animal}年）${lunar.isLeap ? '闰' : ''}${lunarMonths[lunar.month - 1]}月${lunarDays[lunar.day - 1]}`;
}
