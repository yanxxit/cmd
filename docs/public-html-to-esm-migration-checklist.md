# 从现有大 HTML 迁移到 ESM 模块

## 说明
- 本文档与 `public/migration-checklist/` 交互页面共享同一套迁移数据。
- 目标是覆盖 `public/` 下从单 HTML 页面迁移到原生 ESM + ImportMaps 结构的常见路径。
- 如需可勾选版本，请直接访问 `public/migration-checklist/`。
...
## 迁移前检查
先确认迁移边界、回归范围和页面形态，避免误把适合框架化的页面继续塞进静态目录。
...
## 收敛入口 HTML
目标是让 index.html 只保留结构骨架、版本键和模块入口，不再承担全部业务逻辑。
...
## 拆分样式
先把样式层次理顺，后续 JS 模块化时才不容易受隐藏级联影响。
...
## 拆分脚本
按职责拆入口、装配、组件、状态、服务和工具函数，避免一层脚本做所有事情。
...
## 处理依赖与兼容
模块化时最容易出问题的是依赖加载顺序、老全局变量和浏览器缓存。
...
## 验证与交付
迁移不是只要页面能打开，关键交互、刷新行为和开发体验都要回归。
...
## 实际开发中的注意事项
### ImportMaps 与版本号拼接
不要把 alias 和 query string 直接拼在一起做动态导入。浏览器会把 @alias?v=123 当成完整 specifier，而不会先走 importmap 再附加参数。

### 脚本执行时机
原页面若依赖 body 底部 script 的天然执行顺序，拆成模块后要显式保证挂载点已存在，避免模块先执行导致 querySelector 取空。

### 重复绑定事件
模块化后若采用重新 render 再重新 bind 的方式，必须避免重复注册事件监听，否则点击一次会触发多次。

### 样式闪烁与懒加载
基础样式应在首屏同步注入；只把非首屏或重型组件样式交给 style-loader，避免出现明显 FOUC。

### 老全局接口兼容
如果外部页面、书签、测试脚本仍调用 window.xxx 方法，迁移时需要明确哪些 API 要保留桥接层，不能一刀切删除。

### 相对路径漂移
页面从单文件拆到多目录后，图片、下载链接、fetch 接口和 iframe src 的相对路径经常出错，必须逐项回归。

### 本地缓存与持久化
localStorage、sessionStorage、IndexedDB 的 key 若写死在旧脚本里，拆分时要集中收口，避免不同模块写入不一致。

### 错误处理被吞掉
从内联脚本改成模块后，未捕获的 import 或 fetch 错误会直接出现在控制台。需要为关键流程补上用户可感知的错误提示。
...
如本次迁移涉及具体页面逻辑，还应补充该页面最接近的人工验证或自动测试说明。

## 参考资产
- 模板：`harness/templates/public-page-template/`
- 模板预览：`public/public-page-template/`
- 拆分页实战案例：`public/importmap-refactor-demo/`
- 交互式清单：`public/migration-checklist/`
