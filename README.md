...
未达到要求，现修改如下 1. 两个按钮改成选项，true/false  模式，2. 这里的过滤值得是 json 的字段的过滤，这是核心，将 json 粘贴过来后，先执行 json 格式化，在根据 json 的 key 
    进行排序，然后渲染到编辑框中，点击过滤时，修改的是 json 数据，修改完毕后再次渲染到页面 3. 实现后，验证执行结果是否与要求一致